app.service('serviceChat', function ($http, $location) {

    this.getAllUsers = function ($scope, usertoken) {
        console.log(usertoken," mndjkh");
        $http({
            method: 'GET',
            ...
            function successCallback(response) {
                ...
                for (var i = 0; i < (response.data.message).length; i++) {
                    ...
                    if (((localStorage.getItem('userid') == a.senderUserId) && (localStorage.getItem('ruserId') == a.recieverUserId)) || ((localStorage.getItem('userid') == a.recieverUserId && localStorage.getItem('ruserId') == a.senderUserId))) {
                        console.log("local user is ", localStorage.getItem('userid'), "a user is ", a.senderUserId, " local re is ", localStorage.getItem('ruserId'), "  reciver is ", a.recieverUserId);
                        arr.push(a);
                    }
